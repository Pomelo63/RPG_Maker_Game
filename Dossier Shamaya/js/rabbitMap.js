...
Spriteset_Map.prototype.update = function() {
 ...
 if (eval(rabbitMapMVCompat) == false) //Are we using MV? If so, don't do these.
	...
	$gameSwitches.setValue(showMiniMapSwitch, true);
};
...
var isNewMap = 30;
	...
	var _alias_refreshthemap = Actor_Hud.prototype.update;
	Actor_Hud.prototype.update = function () {
		_alias_refreshthemap.call(this);
		miniMapNeedUpdate();
	};
function miniMapNeedHide(){
	if ($gameMessage.isBusy()) {return true};
	return false
};

function miniMapNeedUpdate() {
	if (miniMapNeedHide(false)) {
		$gameScreen._pictures[mapBGPictureID]._opacity -= 15;
		$gameScreen._pictures[mapPictureID]._opacity -= 15;
		$gameScreen._pictures[mapMaskPictureID]._opacity -= 15;
		$gameScreen._pictures[mapPlayerDotPictureID]._opacity -= 15;
	} 
}
